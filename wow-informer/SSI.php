...
function ssi_recentPosts($num_recent = 8, $exclude_boards = null, $output_method = 'echo')
{
	...
	$request = db_query("
		...
		FROM ({$db_prefix}messages AS m, {$db_prefix}topics AS t, {$db_prefix}messages AS mf, {$db_prefix}boards AS b)
...
function ssi_recentTopics($num_recent = 8, $exclude_boards = null, $output_method = 'echo')
{
	...
	$request = db_query("
		...
		WHERE t.ID_LAST_MSG >= " . ($modSettings['maxMsgID'] - 35 * min($num_recent, 5)) . "
			...
			AND ( b.ID_BOARD IN (" . implode(', ', $normal_boards) . ") OR t.isSticky = 1 OR ms.ID_MEMBER = $ID_MEMBER )
...
// Recent topic list:   [board] Subject by Poster	Date  --- Modified by WoW-Informer to list by post creation date
function ssi_recentTopics2($num_recent = 8, $exclude_boards = null, $output_method = 'echo')
{
	global $context, $settings, $scripturl, $txt, $db_prefix, $ID_MEMBER;
	global $user_info, $modSettings, $func;
...
function ssi_quickSearch($output_method = 'echo')
{
	...
	echo '
		<form style="display: inline; margin: 0;" action="', $scripturl, '?action=search2" method="post" accept-charset="', $context['character_set'], '">
			<input type="hidden" name="advanced" value="0" /><input type="text" name="search" size="30" value="Search WoW-Informer.com..." onfocus="this.value=\'\'" /> <input type="submit" name="submit" value="', $txt[182], '" />
		</form>';
}
