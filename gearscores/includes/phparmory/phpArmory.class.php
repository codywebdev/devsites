...
class phpArmory5 {
    ...
    protected function getXmlData($url, $userAgent = NULL, $timeOut = NULL) {
		...
		$queueIpAddress = false;	
		$queueIpAddress = waitInQueue2();
		if(!$queueIpAddress) return false;
		//echo microtime(1).' ** '.$queueIpAddress.'<br />';
			...
			/*include '/home/gearscores/public_html/includes/proxylist.php';
			if ($useProxyListWowarmory) {
			$proxyAddress = $proxylist[array_rand($proxylist)];
			curl_setopt($ch, CURLOPT_PROXY, $proxyAddress);
			}*/
			//$queueIpAddress is set by the waitInQueue() function
			
            curl_setopt ( $ch, CURLOPT_URL, $url );
            curl_setopt ( $ch, CURLOPT_RETURNTRANSFER, 1 );
            curl_setopt ( $ch, CURLOPT_CONNECTTIMEOUT, $timeout );
            curl_setopt ( $ch, CURLOPT_USERAGENT, $userAgent );
			curl_setopt ( $ch, CURLOPT_INTERFACE, $queueIpAddress );
    ...
    public function getCharacterData($characterName = NULL, $realmName = NULL, $onlyBasicData = false) {
		...
		//wait in the queue to avoid querying wow armory too fast
		//$queueIpAddress = '74.124.219.214';
		//if(!waitInQueue()) return false;
        ...
        if (is_string($characterName) && is_string($realmName)) {
            ...
            if (is_array($characterXML) && array_key_exists('XmlData', $characterXML)) {
				...
				$onlyBasicData = false; //we never need this and it queries the server multiple times (bad)
                if ($onlyBasicData) {
				...
				if ($characterArray['characterinfo']['charactertab']['items']['item']['id'] != '') { //only 1 item
					$singleItem[0] = $characterArray['characterinfo']['charactertab']['items']['item'];
					unset($characterArray['characterinfo']['charactertab']['items']['item']);
					$characterArray['characterinfo']['charactertab']['items']['item'] = $singleItem;
				}
				$sizeofCharacter = sizeof($characterArray['characterinfo']['charactertab']['items']['item']);
				for ($i=0;$i<$sizeofCharacter;$i++) {   
					//if item is missing data, then populate from item database
					if ($characterArray['characterinfo']['charactertab']['items']['item'][$i]['id'] > 0 && $characterArray['characterinfo']['charactertab']['items']['item'][$i]['level'] < 1) {
						//----------begin item update
							if ($characterArray['characterinfo']['charactertab']['items']['item'][$i]['id'] > 0) 		
							{
								$query = "SELECT `id`, `updated` FROM `items` WHERE `id` = '".mysql_real_escape_string($characterArray['characterinfo']['charactertab']['items']['item'][$i]['id'])."' LIMIT 1";
								...
								if ($row['id'] > 0) {
									...
									if ($row['updated'] < (time()-(60*60*24*7*2))) {
										$wowItem = getXmlAsArray(file_get_contents('http://www.wowhead.com/item='.$characterArray['characterinfo']['charactertab']['items']['item'][$i]['id'].'&xml'));
						...
						//-----------end item update
						$query = "SELECT * FROM `items` WHERE `id` = '".mysql_real_escape_string($characterArray['characterinfo']['charactertab']['items']['item'][$i]['id'])."' LIMIT 1";
						$result = mysql_query($query);
						$row = mysql_fetch_array($result, MYSQL_ASSOC); 
						$characterArray['characterinfo']['charactertab']['items']['item'][$i]['level'] = $row['level'];
						$characterArray['characterinfo']['charactertab']['items']['item'][$i]['name'] = $row['name'];
						$characterArray['characterinfo']['charactertab']['items']['item'][$i]['icon'] = $row['icon'];
						$characterArray['characterinfo']['charactertab']['items']['item'][$i]['rarity'] = $row['rarity'];
					}
				}
	...
	//created by gearscores.com to obtain raid experience info
    public function getCharacterRaidData($characterName = NULL, $realmName = NULL, $onlyBasicData = false) {
        ...
        if (is_string($characterName) && is_string($realmName)) {
            ...
            $armoryBaseURL = $this->armory."character-";
            $armoryBaseURLEnd = ".xml?r=".urlencode($realmName)."&n=".urlencode($characterName)."&c=14807";

            $characterXML = $this->getXmlData($armoryBaseURL . "statistics" . $armoryBaseURLEnd);
	...
	//created by gearscores.com to obtain raid experience info
    public function getCharacterRaidData2($characterName = NULL, $realmName = NULL, $onlyBasicData = false) {
        ...
        if (is_string($characterName) && is_string($realmName)) {
            ...
            $characterXML = $this->getXmlData('http://us.battle.net/wow/en/character/kelthuzad/calimorph/'); //$armoryBaseURL . "statistics" . $armoryBaseURLEnd);
